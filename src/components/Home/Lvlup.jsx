...
const HowWeElevate = () => {
  ...
  return (
    <section className="relative py-24 bg-white text-center overflow-hidden">
      {/* Kotak besar background oranye */}
      <div className="absolute inset-0 flex justify-center mt-56">
        <div className="bg-orange-500 w-full h-3/5 mt-32 "></div>
      </div>
  ...
  {cards.map((item, index) => (
    <div
      key={index}
      className="relative flex bg-white shadow-lg rounded-xl overflow-hidden hover:shadow-2xl transition-shadow duration-300 mx-auto w-full md:w-3/4"
    >
      {/* Kotak background di dalam card */}
      <div
        className="m-6 md:m-10 w-40 md:w-56 h-40 md:h-56 rounded-md flex-shrink-0"
        style={{
          backgroundImage: `url(${item.image})`,
          backgroundColor: item.color || "#F97316", // fallback oranye
      ...
      <div className="p-6 md:p-10 flex flex-col justify-center text-left">
        ...
        <p className="text-gray-800 text-sm italic border-l-4 border-orange-500 pl-3">
          “{item.quote}”
        </p>
      </div>
    </div>
